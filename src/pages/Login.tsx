...
export default function Login() {
  ...
  const onSubmit = (_values: LoginValues) => onSuccess();
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        // Store minimal profile info if needed
        localStorage.setItem('oauth_provider', 'google');
        localStorage.setItem('oauth_name', userInfo.name || '');
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            ...
            <Button
              ...
              className="w-full h-10 gradient-primary text-white font-medium shadow-glow hover:opacity-90 transition-opacity mt-2"
            >
              {isSubmitting ? (
